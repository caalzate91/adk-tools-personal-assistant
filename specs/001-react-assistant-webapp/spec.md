...
- **Conversation**: An ordered sequence of Messages tied to an authenticated user. Restored from
  Firestore on page load; persists across browser reloads.
...
- The web app source will reside in a dedicated frontend directory (`/personal_assistant/frontend/`), keeping it
  distinct from the Python backend root.
